...
### 9. Links
- [x] **[Architecture]** `SOCIAL_LINKS` imported but bypassed for all but one entry — already caused live drift (`x.com/x_thechrisgrey` vs constant `x.com/thechrisgrey`). Reconcile the X URL, then source all from the constant. Impact: Medium. Effort: Low. (`src/pages/Links.tsx:43-44,5`; `src/constants/links.ts:9`) — added 2026-06-15 — **partially completed 2026-06-15 (WP-E…J): non-X links now sourced from SOCIAL_LINKS + social cards deduped; the personal X handle (x.com/thechrisgrey vs x.com/x_thechrisgrey) is left unresolved pending owner decision**
