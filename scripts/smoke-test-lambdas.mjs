...
async function checkSessionIssuer(endpoint) {
  ...
  try {
    ...
    if (res.status === 200 && serverKey) {
      ...
      record("issued tokens verify for their scope", chat.valid === true && bp.valid === true,
...
async function checkChatRejectsUnauthenticated(endpoint) {
  ...
  try {
    ...
    record("chat enforces auth on unauthenticated request", ok, ok ? "rejected" : `unexpected body: ${body.slice(0, 80)}`);
  } catch (e) {
